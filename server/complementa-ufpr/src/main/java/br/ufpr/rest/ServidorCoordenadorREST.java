...
public class ServidorCoordenadorREST {
    ...
    public ResponseEntity<List<ServidorCoordenadorDTO>> obterTodosServidoresCoordenadores() {
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .body(lista.stream().map(e -> mapper.map(e, ServidorCoordenadorDTO.class)).collect(Collectors.toList()));
    }
    ...
    public ResponseEntity<ServidorCoordenadorDTO> buscaPorId(@PathVariable String id) {
        ...
        if (servidorCoordenador.isEmpty()) {
            ...
            return ResponseEntity.status(HttpStatus.OK).body(mapper.map(servidorCoordenador.get(), ServidorCoordenadorDTO.class));
        }
    }

    @PostMapping
    public ResponseEntity<ServidorCoordenadorDTO> inserirServidorCoordenador(@RequestBody ServidorCoordenador servidorCoordenador) {

        try {
            ServidorCoordenador sc = repo.save(servidorCoordenador);
            Optional<ServidorCoordenador> scOpt = repo.findById(sc.getId().toString());
            if (!scOpt.isPresent()) {
                throw new Exception("Criação do servidor coordenador não foi realizada com sucesso");
            }
            return ResponseEntity.status(HttpStatus.CREATED).body(mapper.map(scOpt.get(), ServidorCoordenadorDTO.class));
        } catch (Exception e) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ServidorCoordenadorDTO> alterarServidorCoordenador(@PathVariable("id") String id, @RequestBody ServidorCoordenador servidorCoordenador) {
